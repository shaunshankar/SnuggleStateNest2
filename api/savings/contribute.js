...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const goalRes = await client.query(
      'SELECT * FROM nest.savings_goals WHERE id=$1 AND household_id=$2',
      [goal_id, auth.householdId]
    )
    ...
    const updatedGoal = await client.query(
      'UPDATE nest.savings_goals SET current_amount=$1 WHERE id=$2 RETURNING *',
      [newAmount, goal_id]
    )

    await client.query(
      `INSERT INTO nest.transactions
         (household_id, user_email, amount, type, category, description, date, notes, created_by)
    ...
    res.json({ contribution: contribRes.rows[0], goal: updatedGoal.rows[0] })
  } catch (err) {
    await client.query('ROLLBACK')
    console.error(err)
