...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      res.json({ household: hhRes.rows[0] || null, members: membersRes.rows })
    } catch (err) {
      console.error(err)
    ...
    try {
      ...
      try {
        ...
        await client.query(
          `UPDATE nest.users SET household_id = $1, role = 'owner' WHERE id = $2`,
          [hh.id, auth.userId]
        )
        await client.query('COMMIT')
        const newToken = signToken({ userId: auth.userId, email: auth.email, householdId: hh.id })
        res.status(201).json({ household: hh, token: newToken })
      } catch (e) {
